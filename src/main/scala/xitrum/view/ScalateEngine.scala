...
object ScalateEngine {
  ...
  def exceptionWithErrorLine(e: Throwable, templateUri: String, templateContent: String = ""): Throwable = {
    ...
    if (file.exists) {
      ...
      val srcLines = srcWithLineNumbers(src.getLines())
      src.close()
  ...
  private def srcWithLineNumbers(lineIt: Iterator[String]): String = {
 ...
 */
class ScalateEngine(templateDirUri: String, allowReload: Boolean, defaultType: String)
  ...
  with Log
{
   ...
   */
  protected def createContext(templateUri: String, engine: STE, currentAction: Action, options: Map[String, Any]): ScalateRenderContext = {
    ...
    currentAction.at.foreach {
      case (k, v) =>
        if (k == ACTION_BINDING_ID || k == CONTEXT_BINDING_ID)
          log.warn(ACTION_BINDING_ID + " and " + CONTEXT_BINDING_ID + " are reserved key names for action's \"at\"")
        else
